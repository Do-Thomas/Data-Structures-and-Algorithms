...
public class N_Queen_Problem {
    ...
    boolean succeeded = false; // Is the problem solved succcessfully?
    ...
    private void recuriveSolve (int index) {
        if(solved)      
            return;    // stop solving immediately
        int i, j;
        
        // Choosing a position for the index queen
        for(i=0; i<positions.size() && !solved; i++) {
            ...
            if(reverseValid(pos,index)) {  // if the position is valid
                ...
                if(index==nQueens-1)  // check completeness, finished
    ...
    private void recuriveSolve() {
        ...
        if(this.size < this.nQueens)
            ...
            solution = new ArrayList();  // Intiate solution
